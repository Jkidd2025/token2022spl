...
const RPC_ENDPOINTS: RPCEndpoint[] = [
  {
    name: 'Helius',
    url: process.env.HELIUS_API_KEY ? `${process.env.HELIUS_RPC_URL}${process.env.HELIUS_API_KEY}` : '',
    priority: 1,
  },
  {
    name: 'GenesysGo',
...
export async function confirmTransaction(
  ...
  try {
    ...
    console.log(`Transaction ${signature} confirmed in ${duration}ms with ${commitment} commitment`);
    
    return 'value' in result && result.value.err === null;
  } catch (error) {
...
export async function getConnection(
  ...
  const availableEndpoints = RPC_ENDPOINTS.filter(endpoint => endpoint.url)
  ...
  for (const endpoint of availableEndpoints) {
    try {
      ...
      if (isHealthy) {
        console.log(`Successfully connected to ${endpoint.name} RPC with ${confirmStrategy.commitment} commitment`);
        return connection;
      }
  ...
  for (const endpoint of availableEndpoints) {
    ...
    try {
      ...
      if (isHealthy) {
        ...
        console.log(`Added ${endpoint.name} RPC to redundant connections with ${confirmStrategy.commitment} commitment`);
      }
    } catch (error) {
      console.error(`Error connecting to ${endpoint.name} RPC:`, error);
      continue;
