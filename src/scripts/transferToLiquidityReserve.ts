...
async function transferToLiquidityReserve() {
  try {
    ...
    // Get the mint and liquidity reserve addresses
    const mintPubkey = new PublicKey(process.env.TOKEN_MINT_ADDRESS!);
    const liquidityReservePubkey = new PublicKey(process.env.LIQUIDITY_RESERVE_ADDRESS!);
    ...
    console.log('Creating/getting liquidity reserve token account...');
    const liquidityReserveTokenAccount = await getOrCreateAssociatedTokenAccount(
      connection,
      operationsWallet,
      mintPubkey,
      liquidityReservePubkey,
      false,
      'confirmed',
      { commitment: 'confirmed' }
    );

    console.log('Liquidity reserve token account:', liquidityReserveTokenAccount.address.toString());
    ...
    const transferAmount = Number(process.env.LIQUIDITY_RESERVE_AMOUNT!) * decimalMultiplier;
    console.log(`Transferring ${process.env.LIQUIDITY_RESERVE_AMOUNT} tokens (${transferAmount} raw amount) to Liquidity Reserve...`);

    const transaction = new Transaction().add(
      createTransferInstruction(
        operationsTokenAccount.address,
        liquidityReserveTokenAccount.address,
        operationsWallet.publicKey,
        transferAmount,
        [],
        TOKEN_PROGRAM_ID
    ...
    console.error('Error in transferToLiquidityReserve:', error);
...
export { transferToLiquidityReserve }; 